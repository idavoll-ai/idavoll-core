...
### Agent 交互式创建流程
用户通过多轮引导式自然对话，逐步描述想要的 Agent 人设与风格，系统实时解析并结构化存储，对话完成后自动生成一份标准化的 agents.md 文档，作为该 Agent 的可持久化配置文件。

整体流程：
1. 启动 Agent 创建对话
2. 引导用户填写 Identity 层（角色、背景、目标）
3. 引导用户填写 Voice 层（语气、口癖、语言、示例对话）
<!-- 实时预览结构化 Profile -->
4. 确认生成 → 输出 agents.md

系统按固定顺序发起引导，避免信息缺失，同时允许用户随时补充、修改。
阶段 1：Identity 信息收集
 - 请描述这个 Agent 的身份 / 角色：
 - 简单介绍它的背景故事（经历、专长）：
 - 这个 Agent 的核心目标 / 对话目的是什么？
阶段 2：Voice 风格收集
 - 希望它用什么语气说话？（正式 / 学术 / 轻松 / 俏皮 / 严肃等）
 - 有没有特别的说话习惯 / 口头禅 / 风格癖好？
 - 使用语言：中文 / 英文 / 其他？
 - 请提供 1～3 组示例问答，让我学习它的说话方式：
    用户问：
    Agent 答：
阶段 3：确认与修正
 - 系统实时回显结构化内容：
 - 我已为你生成 Agent 人设如下，是否需要修改？（展示 YAML 结构预览）
 - 确认无误后进入生成。

[!IMPORTANT] 最终生成的文件结构 参考下文的 **双层 Profile**
