...
│   MemoryManager  SkillsLibrary  SessionSearch(stub)             │
│   ContextCompressor  HookBus  Scheduler                         │
├─────────────────────────────────────────────────────────────────┤
│                    Profile Workspace (Disk)                     │
│   SOUL.md  MEMORY.md  USER.md  PROJECT.md  skills/              │
└─────────────────────────────────────────────────────────────────┘
```
...
workspaces/
  {uuid}/
    ...
    # 跨会话原始对话改由 SQLite session_records 存储
```
...
监听 **`review.completed`** 事件，驱动外部成长闭环：
...
话题参与（generate_response）
  │
  ├─ 对话中 Agent 主动调用 memory 工具 → 写入 MEMORY.md / USER.md
  │
  ▼
topic.closed / on_session_end
  │
  ├─ VingolfApp 将原始对话持久化到 SQLite session_records
  │    └─ 后续 SQLiteSessionSearch 可跨 Session 召回
  │
  └─ 下次 Session 启动时：
       MemoryManager.system_prompt_block() → 历史事实冻结进 System Prompt
       MemoryManager.prefetch(query)       → 当轮相关记忆动态注入
       SQLiteSessionSearch.search()        → 跨 Session 经验召回
```
...
Session 结束（close_session / topic.closed）
  └─ ExperienceConsolidator.run(agent, session)
       └─ 更新 MEMORY.md / USER.md / skills/，并将原始对话写入 SQLite
```
...
│  │    ├─ persist raw session record emit("on_pre_compress")              │ │
│  │    └─ detect & save skills                                            │ │
│  │                                                                       │ │
│  └───────────────────────────────────────────────────────────────────────┘ │
└────────────────────────────────────────────────────────────────────────────┘
...
4. _attach_runtime(agent, workspace)
   ...
   ├─ agent.session_search = SessionSearch()  # product layer replaces with SQLiteSessionSearch
...
```
数据来源：SQLite `session_records.conversation`
  由产品层在 session 关闭或 topic 关闭时写入

检索策略（MVP，无嵌入）：
  关键词分词 → 对每个原始 Session 记录打分
  → 选出 top-3 后按需总结（受 token_budget=300 约束）
